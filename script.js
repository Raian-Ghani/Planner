...
requestAnimationFrame(function anim(){
    ...
    blobs.forEach(b => {
        ...
        if (d < 120) { let f = (120 - d) / 120; let a = Math.atan2(dy, dx); b.vx -= Math.cos(a) * f * 1.5; b.vy -= Math.sin(a) * f * 1.5; }
        b.vx += (b.baseX - b.x) * 0.05; b.vy += (b.baseY - b.y) * 0.05; b.vx *= 0.85; b.vy *= 0.85; b.x += b.vx; b.y += b.vy;
        ctx.beginPath(); ctx.arc(b.x, b.y, b.radius, 0, Math.PI*2); ctx.fillStyle = b.color; ctx.globalAlpha = 0.25; ctx.fill();
    });
...
// --- THE SCIENTIFIC EXPONENT 'E' INTERCEPT BANNER ---
...
function renderDirectives() {
    ...
    taskMatrix.forEach((item) => {
        ...
        let timeString = "";
        if (displayMins > 0) {
            timeString = displaySecs > 0 ? `${displayMins}m ${displaySecs}s` : `${displayMins}m`;
        } else {
            timeString = `${displaySecs}s`;
        }
...
button.addEventListener('click', () => {
    ...
    // --- UPDATED DECIMAL PERCENTAGE-BASED SECONDS ENGINE ---
    let totalSeconds = 0;
    if (timeRawString.includes('.')) {
        ...
        // Pad out to ensure standard handling (.5 becomes 50, .05 stays 05)
...
// --- TERMINAL WORK FOCUS CONTROL (TIMER 2) ---
...
// --- PERSISTENT SEVEN-DAY REWARD INVENTORY DECK ---
...
function renderBreakBankShelf() {
    ...
    breakRegistry.forEach(b => {
        ...
        let actionControls = b.state === 'saved' 
            ? `<button class="btn-use" onclick="triggerBreakActivation('${b.id}')">Use</button>` 
            : `<span></span>`;
...
addInterestBtn.addEventListener('click', () => { const v = interestInput.value.trim(); if (v && !savedInterests.includes(v)) { savedInterests.push(v); localStorage.setItem('userVectors', JSON.stringify(savedInterests)); renderTags(); interestInput.value = ''; } });

// Genesis run configurations
